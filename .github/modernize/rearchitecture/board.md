...
- 🔄 t13.1 [backend] **Follow-up** — close the enforcement gap and restore correlation logging (dispatched 10:05Z) [deps: t13] — **G1 (HIGH)** no rule enforces `presentation ⊥ infrastructure` (verified: only Rule 5 names both, and it constrains *application*); 2 live violations were hand-fixed, so the rule is load-bearing. **G2 (HIGH)** `AgentReviewExecutor` switched SLF4J→`java.util.logging`, losing virtual-thread MDC propagation; the guarding tests were deleted rather than the behaviour restored — to be reintroduced as a logging/correlation **port**. **G3 (MEDIUM)** duplicate `ConfigDefaults`/`RetryPolicyUtils`.

### Phase: Hardening
- ⏳ t14 [tester] Full regression test run (T015) [deps: t13.1] — repointed: must exercise the correlation-logging capability t13.1/G2 restores
...
### Phase: Review
- ⏳ t17 [architect] Architecture review — verify layered structure matches design [deps: t13.1, t16] — repointed: the `presentation ⊥ infrastructure` rule (t13.1/G1) must exist before the layered structure can be certified
