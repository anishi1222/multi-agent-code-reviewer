...
- ❌ failed[findings] t16.2 [architect] **Follow-up** — rule on ADR-0006 D3's false premise and record the fifth direction inversion (01:16:34Z→01:23:54Z, 7m 20s) [deps: t16.1] — **0 CRITICAL, 1 HIGH.** The architecture-of-record work is complete and internally verified (13/13 assertions; no source/build/runtime files changed): D3 now states that only `ApplicationPortFactory` is a Micronaut `@Factory`, rejects blanket relocation, and records deviation #8 with a non-concealing end-state contract. The HIGH is the open review-path inversion itself: Micronaut still resolves infrastructure `ReviewOrchestratorFactory` as inbound `RunReviewPort`, hidden by Rule 4's named exemption. Per §3.2.1 this is a pipeline FAIL until **t16.3** implements the contract and t16.2 is re-dispatched to a fresh clean PASS.
- 🔄 t16.3 [backend] **Remediation** — eliminate ADR-0006 deviation #8: bind `RunReviewPort` to an application-layer implementation, separate review-path config/adapter construction into outbound boundaries and composition-root wiring, remove `ReviewOrchestratorFactory` from Rule 4's exemption, and preserve CLI behavior. Must pin both the implementing layer and the actual Micronaut DI binding, with a RED-first non-vacuity control; a file move or partial extraction is explicitly not a remedy. (dispatched 2026-08-07T01:25:12Z) [remediates: t16.2]

### Phase: Review
- ⏳ t17 [architect] Architecture review — verify layered structure matches design [deps: t13.1, t16, t16.1, t16.2, t16.3] — repointed four times: the `presentation ⊥ infrastructure` rule (t13.1/G1) must exist before the layered structure can be certified; t16 found 4 open layer defects (t16.1); and t16.2 surfaced deviation #8, which requires t16.3 remediation followed by a fresh clean t16.2 PASS.
