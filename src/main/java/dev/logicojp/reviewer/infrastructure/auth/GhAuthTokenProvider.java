...
final class GhAuthTokenProvider {
    ...
    private static final String[] TOKEN_ENV_VARS = {"GITHUB_TOKEN", "GH_TOKEN", "GH_ENTERPRISE_TOKEN"};
    ...
    GhAuthTokenProvider(long timeoutSeconds, Supplier<String> ghPathSupplier, GhAuthCommand ghAuthCommand) {
    ...
    Optional<String> resolve() {
        ...
        for (int attempt = 1; attempt <= GH_AUTH_MAX_ATTEMPTS; attempt++) {
            ...
            if (attempt < GH_AUTH_MAX_ATTEMPTS && !sleepBeforeRetry(attempt)) return Optional.empty();
        }
        return Optional.empty();
    }

    private boolean sleepBeforeRetry(int attempt) {
        long backoffMs = RetryPolicyUtils.computeBackoffWithJitter(GH_AUTH_BACKOFF_BASE_MS, GH_AUTH_BACKOFF_MAX_MS, attempt);
        logger.debug("gh auth token failed (attempt {}/{}), retrying in {}ms", attempt, GH_AUTH_MAX_ATTEMPTS, backoffMs);
        try {
    ...
    private static Optional<String> attemptResolveFromGhAuth(String ghPath, long timeoutSeconds) {
        Path executionPath = CliPathResolver.revalidateExecutionPath(ghPath, "gh").orElse(null);
    ...
    private static void scrubSensitiveTokenEnvironment(ProcessBuilder builder) {
        for (String envVar : TOKEN_ENV_VARS) builder.environment().remove(envVar);
    }
