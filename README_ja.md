...
- 2026-05-28 (v2026.05.28-ci-release-hardening): CI とリリース運用の強化 — GitHub Actions workflow 既定権限を `permissions: {}` にし、ビルド系ジョブは `contents: read`、リリース作成ジョブのみ `contents: write` へ限定。`GH_REPO` 明示によりリリースジョブの不要な checkout を削除し、`Supply Chain Guard` から重複していた OWASP Dependency Check を外して詳細監査を `Dependency Audit` に集約。CodeQL Java/Kotlin 解析を `build-mode: none` に変更し、Dependency Submission 権限、Actions/Maven プラグイン依存関係、`CopilotCliPathResolver` テストを更新。
