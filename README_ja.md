...
2026-02-16 〜 2026-05-28 のレビューサイクルで検出された全指摘事項は対応済みです。
...
このプロジェクトは `com.github:copilot-sdk-java:0.3.0-java.2` に依存しています。SDK artifact と upstream repository は MIT License を宣言しており、SDK コード自体はサーバーサイド組み込み、改変、再配布に使いやすい permissive license です。
...
reviewer:
  ...
  rubber-duck:
    enabled: false                   # Rubber-duck peer discussion モードをグローバルに有効化
    dialogue-rounds: 2               # 対話ラウンド数（1〜10）
    peer-model: ${RUBBER_DUCK_PEER_MODEL:}  # ピアモデル（環境変数または明示指定）
...
**エージェント単位の設定:** `.agent.md` のフロントマターでエージェントごとに rubber-duck 設定を指定できます（エージェント定義ファイルセクションを参照）。
...
Rubber-duck モード用の追加フロントマターフィールド:
