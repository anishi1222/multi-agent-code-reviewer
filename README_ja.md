...
2026-02-16 〜 2026-06-08 のレビューサイクルで検出された全指摘事項は対応済みです。
...
- **GraalVM**（任意、`-Pnative` の Native Image ビルド時のみ必須）
...
- `Dependency Audit` が `mvn -Psecurity-audit verify` による OWASP `dependency-check-maven` 実行を担当します。
...
└── src/main/java/dev/logicojp/reviewer/
    ...
    │   ├── ReviewSessionMessageSender.java # SDK sendAndWait ラッパー
    ...
    │   ├── DialogueRound.java           # Rubber-duck 対話ラウンドレコード
    │   ├── RubberDuckDialogueExecutor.java # Rubber-duck 2モデル対話実行
    │   └── SynthesisStrategy.java       # Rubber-duck 統合戦略（sealed interface）
    ├── cli/
    ...
    │   ├── RubberDuckConfig.java        # Rubber-duck peer discussion 設定
    ...
    │   ├── CopilotHealthProbe.java      # SDK getStatus / getAuthStatus プローブ
